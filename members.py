...
st.markdown('<h2 class = "main-title">WORKER DEMOGRAPHIC DISTRIBUTION VIEW</h2>', unsafe_allow_html=True)
...
if not filtered_df.empty:  
 ...
 # Group by month and educational background, then count the number of workers
    monthly_workers = df.groupby(['Year', 'Gender']).size().unstack().fillna(0)
...
# Count the occurrences of each Status
    ...
    educ_counts = df["Educational Background"].value_counts().reset_index()
    educ_counts.columns = ["education", "Count"]

    with cls1:
        # Display the header
        st.markdown('<h3 class="custom-subheader">Educational Background Distribution of Workforce</h3>', unsafe_allow_html=True)

        # Create a donut chart
        fig = px.pie(educ_counts, names="education", values="Count", template="plotly_dark", color_discrete_sequence=custom_colors)
        fig.update_traces(textposition='inside', textinfo='value+percent')
        fig.update_layout(height=450, margin=dict(l=0, r=10, t=30, b=50))
        ...
        st.plotly_chart(fig, use_container_width=True)
# Count the occurrences of each Status
    type_counts = df["Skilled or Unskilled worker"].value_counts().reset_index()
    type_counts.columns = ["type", "Count"]

    with cls2:
        # Display the header
        st.markdown('<h3 class="custom-subheader">Type of Workforce</h3>', unsafe_allow_html=True)

        # Create a donut chart
        fig = px.pie(type_counts, names="type", values="Count", hole=0.5, template="plotly_dark", color_discrete_sequence=custom_colors)
        fig.update_traces(textposition='inside', textinfo='value+percent')
        fig.update_layout(height=450, margin=dict(l=0, r=10, t=30, b=50))
    ...
    # Create a pivot table to get the sum of child dependents for each worker's age range
    age_range_dep = filtered_df.pivot_table(
            index='Worker age range',
            values='Number of child dependents',
            aggfunc='sum',
            fill_value=0
        )

    # Identify workers with spouses
    spouse_dep = filtered_df[filtered_df['Spouse dependents'] == 'Yes'].pivot_table(
            index='Worker age range',
            values='Number of child dependents',
            aggfunc='sum',
            fill_value=0
    ...
    with cls2:
        ...
        fig_dependents = go.Figure()

        # Create the bar chart for all workers
        fig_dependents.add_trace(go.Bar(
            x=age_range_dep.index,
            y=age_range_dep['Number of child dependents'],
            name='All Workers',
            textposition='inside',
            textfont=dict(color='white'),
            hoverinfo='x+y+name',
        ...
        # Create the bar chart for workers with spouses
        fig_dependents.add_trace(go.Bar(
            x=spouse_dep.index,
            y=spouse_dep['Number of child dependents'],
            name='Workers with Spouses',
            textposition='inside',
            textfont=dict(color='white'),
            hoverinfo='x+y+name',
            marker_color='#E66C37'
        ...
        fig_dependents.update_layout(
            barmode='group',  # Grouped bar chart
            xaxis_title="Worker's Age Range",
            yaxis_title="Number of Child Dependents",
            font=dict(color='Black'),
            xaxis=dict(title_font=dict(size=14), tickfont=dict(size=12)),
            yaxis=dict(title_font=dict(size=14), tickfont=dict(size=12)),
            margin=dict(l=0, r=0, t=30, b=50),
        ...
        # Display the chart in Streamlit
        st.markdown('<h3 class="custom-subheader">Number of Child Dependents by Worker\'s Age Range and Spousal Status</h3>', unsafe_allow_html=True)
...
# Count the occurrences of each Status
    ...
    with clus2:
        ...
        # Create a donut chart
        fig = px.pie(spouse_gender_counts, names="Status", values="Count", template="plotly_dark", color_discrete_sequence=custom_colors)
        fig.update_traces(textposition='inside', textinfo='value+percent')
        fig.update_layout(height=450, margin=dict(l=0, r=10, t=30, b=50))
...
# Count the occurrences of each Status
    ...
    with clus1:
        ...
        # Create a donut chart
        fig = px.pie(em_counts, names="dependents", values="Count", hole=0.5, template="plotly_dark", color_discrete_sequence=custom_colors)
        fig.update_traces(textposition='inside', textinfo='value+percent')
        fig.update_layout(height=450, margin=dict(l=0, r=10, t=30, b=50))
